...
* - 배열과 달리 크긱가 고정되지 않아 필요에 따라 확장한다.
...
*
* 5. 성능
* - 요소 추가 시, 리스트의 끝에 추가되는 경우 0(1)의 시간 복잡도를 가진다.
...
* - 메모리 부족 상황에서 자동으로 배열을 재합당하기 때문에 성능 저하가 발생할 수 있다.
 */
public class Application {
    public static void main(String[] args) {
        List<String>  booklist = new ArrayList<>(); // booklist는 List 기능까지만 가진다
        booklist.add("자바의 정석");
        booklist.add("클린 코드");
        booklist.add("자바의 정석"); // list는 중복 허용
        ...
        arrayBooks[1] = "클린 코드";
//        arrayBooks[2] = "추가 불가"; // Error : ArrayIndexOutOfBoundsException
        System.out.println("arrayList: " + arrayBooks); // 주소
        System.out.println("Array: " + Arrays.toString(arrayBooks));

        booklist.add("A");
        System.out.println("ArrayList: " + booklist);
        Collections.sort(booklist); //Collections 의 static 메서드 sort(정렬)
        System.out.println("ArrayList: " + booklist);

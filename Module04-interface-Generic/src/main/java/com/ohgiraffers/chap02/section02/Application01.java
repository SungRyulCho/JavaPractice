 ...
 * - 비유: 상자(box)를 만들때, 안에 넣을 물건(타입)을 나중에 정하는 것이다.
 *
 * 제네릭스가 필요한 이유
 * - 제네릭스 없이 상자를 만들면 모든 물건(Object)는 넣을 수 있지만
 * - 꺼낼 때 형변환이 필요하다 -> 실수하면 런타임 오류
 * - 예 : Object box 에 String 을 넣고 꺼낼 때 integer로 형변환하면 오류
 * - 제네릭스는 타입을 미리 정해서 이런 실수를 컴파일 시점에 잡아준다
 *
 * 장점:
 * - 초급 : 잘못된 물건 못넣게 해서 실수를 줄임
 * - 중급 : 형변환 하지않고 사용할 수 있어서 좋음
...
//        stringBox.setItem(12133);
        ...
        String result2 = stringBox.getItem();
